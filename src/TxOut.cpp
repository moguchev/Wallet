...
TxOut::TxOut(double value, Script&& pk_script) {
  value_ =
      static_cast<int64_t>(SATOSHI_COEF * value);  // convert BTC to Satoshi
...
std::vector<byte> TxOut::get_byte_output() const {
  ...
  const auto script_lth =
      cu::to_varint_byte(static_cast<uint64_t>(pk_script_.get_length()));
...
TxOut TxOut::from_data(const std::vector<byte>& bytes) {
  uint64_t value =
      cu::to_type<uint64_t>(bytes.begin(), bytes.begin() + sizeof(uint64_t));
