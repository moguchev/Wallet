...
void TxIn::set_previous_output(const std::string& prev_tx_hash,
                               uint32_t index) {
...
TxIn TxIn::from_data(const std::vector<byte>& data) {
  ...
  out_point.set_index(
      cu::to_type<uint32_t>(data.begin() + 32, data.begin() + 36));
...
std::vector<byte> TxIn::get_byte_input() const {
  ...
  const auto script_lth =
      cu::to_varint_byte(static_cast<uint64_t>(script_.get_length()));
...
void TxIn::sign_by(const std::string& private_key_wif,
  ...
  auto sig = cu::signature_to_der_byted(cu::sign(private_key_wif, tx));
